var A = { a: 7, c: 6, h: 1, l: 2, m: 2, q: 4, s: 4, t: 2, v: 1, z: 0 }, V = /([astvzqmhlc])([^astvzqmhlc]*)/gi, Z = /-?[0-9]*\.?[0-9]+(?:e[-+]?\d+)?/gi;
function _(o) {
  const n = o.match(Z);
...
function U(o) {
  ...
  return t[0] !== "M" && t[0] !== "m" || t.replace(V, (l, r, a) => {
    ...
    if (c === "m" && s.length > 2 && (n.push([i, ...s.splice(0, 2)]), c = "l", i = i === "m" ? "l" : "L"), s.length < A[c]) return "";
    for (n.push([i, ...s.splice(0, A[c])]); s.length >= A[c] && s.length && A[c]; ) n.push([i, ...s.splice(0, A[c])]);
    return "";
  }), n;
}
function B(o, n) {
...
function F(o, n) {
  ...
  for (let h = 0; h < n.length; ++h) {
    ...
    switch (b) {
      ...
      case "a":
      case "A":
        if (e = n[h], M === null) throw new Error("This should never happen");
        b === "a" ? (t += e[6], l += e[7]) : (t = e[6], l = e[7]), p = e[1], f = e[2], k = e[3] * Math.PI / 180, s = !!e[4], c = !!e[5], i = { x: t, y: l }, u = { x: (M.x - i.x) / 2, y: (M.y - i.y) / 2 }, B(u, -k), q = u.x * u.x / (p * p) + u.y * u.y / (f * f), q > 1 && (q = Math.sqrt(q), p *= q, f *= q), R = { x: p * u.y / f, y: -(f * u.x) / p }, L = p * p * f * f, z = p * p * u.y * u.y + f * f * u.x * u.x, c !== s ? C(R, Math.sqrt((L - z) / z) || 0) : C(R, -Math.sqrt((L - z) / z) || 0), a = Math.atan2((u.y - R.y) / f, (u.x - R.x) / p), r = Math.atan2(-(u.y + R.y) / f, -(u.x + R.x) / p), B(R, k), j(R, (i.x + M.x) / 2, (i.y + M.y) / 2), o.save(), o.translate(R.x, R.y), o.rotate(k), o.scale(p, f), o.arc(0, 0, 1, a, r, !c), o.restore();
        break;
      case "C":
...
function H(o, n, t, l, r = 0) {
  if (typeof r == "number" && (r = [r]), Array.isArray(r)) {
    if (r.length === 0 || r.length > 4) throw new RangeError(`Failed to execute 'roundRect' on '${this.constructor.name}': ${r.length} radii provided. Between one and four radii are necessary.`);
    r.forEach((k) => {
      if (k < 0) throw new RangeError(`Failed to execute 'roundRect' on '${this.constructor.name}': Radius value ${k} is negative.`);
    });
  } else return;
  if (r.length === 1 && r[0] === 0) {
    this.rect(o, n, t, l);
