...
function we({ schema: s, initial: p, isNew: a, canDelete: m, issueKey: b, creatorId: o, lastChangedById: v, reactions: _, comments: x, currentUserId: k, onSave: z, onDelete: I, onCancel: i, onReact: d, onComment: C }) {
  ...
  return e.jsx("div", { style: t.overlay, role: "dialog", "aria-modal": "true", "aria-label": a ? "New issue" : "Edit issue", onKeyDown: J, children: e.jsxs("div", { style: t.dialog, children: [e.jsxs("div", { style: t.dialogHeader, children: [e.jsx("strong", { style: { fontSize: 15 }, children: a ? "New Issue" : "Edit Issue" }), e.jsx("button", { onClick: i, style: { ...t.btn, border: "none", background: "none", fontSize: 18, padding: "0 4px" }, "aria-label": "Close", children: "\u2715" })] }), e.jsxs("div", { style: t.dialogBody, children: [!a && o && e.jsxs("div", { style: { display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 8, fontSize: 12, color: "#777" }, children: [e.jsxs("span", { children: ["Created by ", e.jsx("strong", { children: Q(o) })] }), v && v !== o && e.jsxs("span", { children: ["Last changed by ", e.jsx("strong", { children: Q(v) })] })] }), s.fields.map((l, L) => {
    const ee = String(D[l.key] ?? ""), W = { id: `wf-${l.key}`, value: ee, onChange: ($) => j((te) => ({ ...te, [l.key]: $.target.value || void 0 })), style: t.input, ref: L === 0 ? B : void 0 };
...
function Ie({ widgetApi: s }) {
  ...
  u.useEffect(() => {
    ...
    async function f() {
      var _a, _b, _c;
      try {
        const [y, c, h, F, M, oe] = await Promise.all([s.readStateEvents("eu.kiefte.issues.schema", 1), s.readStateEvents("eu.kiefte.issue", 1e4), s.readStateEvents("m.room.power_levels", 1), s.readRoomEvents("m.room.message", 2e3).catch(() => []), s.readRoomEvents("m.reaction", 5e3).catch(() => []), s.readRoomEvents("eu.kiefte.issue", 1e4).catch(() => [])]);
        if (n) return;
        ...
        K(F.filter((w) => w.content["eu.kiefte.issue_id"])), U(M);
      } catch (y) {
        n || z(String(y));
      } finally {
        n || x(false);
    ...
    const n = `action:${ae.WidgetApiToWidgetAction.SendEvent}`, f = (y) => {
      ...
      if (c == null ? void 0 : c.type) if (c.type === "eu.kiefte.issues.schema") {
        ...
        const h = c.content;
        (h == null ? void 0 : h._deleted) ? b((F) => F.filter((M) => M.stateKey !== c.state_key)) : b((F) => {
          const M = F.find((q) => q.stateKey === c.state_key);
          return [...F.filter((q) => q.stateKey !== c.state_key), { stateKey: c.state_key, eventId: c.event_id ?? "", content: h, sender: c.sender, creatorId: (M == null ? void 0 : M.creatorId) ?? c.sender, ts: c.origin_server_ts }];
        });
      } else c.type === "m.reaction" ? U((h) => [...c.event_id ? h.filter((M) => M.event_id !== c.event_id) : h, c]) : c.type === "m.room.message" && c.content["eu.kiefte.issue_id"] && K((h) => [...c.event_id ? h.filter((M) => M.event_id !== c.event_id) : h, c]);
    };
    return s.on(n, f), () => {
      s.off(n, f);
    };
  ...
  }, [s, m]), W = u.useMemo(() => m.filter((n) => !n.content._deleted).sort((n, f) => f.ts - n.ts), [m]), $ = d !== null && d !== "new" ? d : null, te = ($ == null ? void 0 : $.content) ?? {}, H = ($ == null ? void 0 : $.stateKey) ?? null;
