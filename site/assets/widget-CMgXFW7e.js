...
function Z({ initial: n, onSave: c, onCancel: l, titleText: k }) {
  var _a;
  const [S, i] = Y.useState(() => n.fields.map(ye)), [m, C] = Y.useState(((_a = n.fields[0]) == null ? void 0 : _a.key) ?? null), [g, u] = Y.useState(false), [h, w] = Y.useState(null), y = S.find((o) => o.key === m) ?? null, f = (o, b) => i((K) => K.map((I) => I.key === o ? { ...I, ...b } : I)), j = (o) => {
    i((b) => {
      var _a2;
      const K = b.filter((I) => I.key !== o);
...
function me({ schema: n, initial: c, isNew: l, canDelete: k, issueKey: S, reactions: i, comments: m, currentUserId: C, onSave: g, onDelete: u, onCancel: h, onReact: w, onComment: y }) {
  const [f, j] = d.useState({ ...c }), [T, R] = d.useState(false), [F, D] = d.useState(""), [M, o] = d.useState(""), [b, K] = d.useState(false), I = d.useRef(null), P = d.useRef(null);
  d.useEffect(() => {
    var _a;
  ...
  return e.jsx("div", { style: t.overlay, role: "dialog", "aria-modal": "true", "aria-label": l ? "New issue" : "Edit issue", onKeyDown: U, children: e.jsxs("div", { style: t.dialog, children: [e.jsxs("div", { style: t.dialogHeader, children: [e.jsx("strong", { style: { fontSize: 15 }, children: l ? "New Issue" : "Edit Issue" }), e.jsx("button", { onClick: h, style: { ...t.btn, border: "none", background: "none", fontSize: 18, padding: "0 4px" }, "aria-label": "Close", children: "\u2715" })] }), e.jsxs("div", { style: t.dialogBody, children: [n.fields.map((r, B) => {
    const A = String(f[r.key] ?? ""), L = { id: `wf-${r.key}`, value: A, onChange: (_) => j((H) => ({ ...H, [r.key]: _.target.value || void 0 })), style: t.input, ref: B === 0 ? I : void 0 };
  ...
  d.useEffect(() => {
    ...
    async function p() {
      ...
      try {
        ...
        l((N == null ? void 0 : N.fields) ? N : null), S(a.filter((E) => {
          var _a2;
          return E.state_key && !((_a2 = E.content) == null ? void 0 : _a2._deleted);
        }).map((E) => ({ stateKey: E.state_key, eventId: E.event_id ?? "", content: E.content, sender: E.sender, ts: E.origin_server_ts })));
        const V = (_b = v[0]) == null ? void 0 : _b.content;
        if (V && b) {
          const E = ((_c = V.users) == null ? void 0 : _c[b]) ?? V.users_default ?? 0, ae = V.state_default ?? 50;
          m(E >= ae);
        } else m(true);
        D(W.filter((E) => E.content["eu.kiefte.issue_id"])), o($);
      } catch (x) {
        s || h(String(x));
      } finally {
        s || g(false);
...
const oe = new URLSearchParams(window.location.search), ke = oe.get("widgetId") ?? void 0, re = oe.get("parentUrl") ?? void 0, z = new ee.WidgetApi(ke, re);
...
let le = false;
z.once("ready", () => {
  le = true, te(document.getElementById("widget-root")).render(e.jsx(ve, { widgetApi: z }));
});
setTimeout(() => {
  if (le) return;
