...
const r = function(r2, i, a) {
    ...
    o = l(i.map((i2) => {
      ...
      if (a)
        for (let e2 = r3.length - 1; e2 >= 0; e2--) {
          let t2 = r3[e2];
          if (t2.href === i2 && (!o2 || t2.rel === `stylesheet`))
            return;
        }
      else if (document.querySelector(`link[href="${i2}"]${s3}`))
        return;
      let l2 = document.createElement(`link`);
      if (l2.rel = o2 ? `stylesheet` : e, o2 || (l2.as = `script`), l2.crossOrigin = ``, l2.href = i2, c && l2.setAttribute(`nonce`, c), document.head.appendChild(l2), o2)
  ...
  return o.then((e2) => {
    for (let t2 of e2 || [])
      t2.status === `rejected` && s(t2.reason);
    return r2().catch(s);
  });
};
export {
